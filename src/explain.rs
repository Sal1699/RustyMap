...
const GLOSSARY: &[(&str, &str)] = &[
    ...
    ("--exclude", "esclude target da una lista o file"),
    ("--exclude-ports", "esclude porte specifiche dallo scan"),
    ("--iR", "genera N target IPv4 pubblici random (necessita --internet-consent)"),
    ("--internet-consent", "conferma autorizzazione per --iR"),
    ...
    ("--sW", "TCP Window scan (RST window heuristic)"),
    ("--sM", "TCP Maimon scan (FIN+ACK, BSD-derived)"),
    ("--sY", "SCTP INIT scan"),
    ("--sZ", "SCTP COOKIE-ECHO scan"),
    ("--sO", "IP protocol scan"),
    ("--sI", "Idle/zombie scan via spoofed IPID"),
    ("--sR", "RPC portmap dump (TCP/UDP 111)"),
    ("--sL", "list scan: risolve target con PTR e esce"),

    // Host discovery (NEW BUG v0.66.6 — these were missing)
    ("--Pn", "skip host discovery — tratta tutti come up"),
    ("--PR", "ARP discovery (auto su LAN, esplicito altrove)"),
    ("--PE", "ICMP echo ping"),
    ...
    ("--os", "OS fingerprint via TCP/IP stack quirks"),
    ("--version-light", "service detection veloce (intensity 2)"),
    ("--version-all", "service detection completa (intensity 9)"),
    ("--version-intensity", "service detection aggressiveness 0-9"),
    ...
    ("--reason", "annota per-port il motivo dello stato finale"),
    ("-n", "no DNS — IP-only"),
    ("--no-dns", "no DNS — IP-only"),
    ...
    ("--oA", "scrive tutti i formati (txt/json/html/md/xml/gnmap)"),
    ("--oN", "scrive output testuale human-readable (.nmap-style)"),
    ("--oG", "scrive output grepable (.gnmap)"),
    ("--oJ", "scrive output JSON"),
    ("--oH", "scrive report HTML"),
    ("--oM", "scrive report Markdown"),
